...
A python library that uses browser automation to scrape news feed from linkedin
...
```sh
pip install linkedin-feed-scraper
```

### Import

```javascript
from linkedin-feed-scraper import *
```
...
To scrape news feed of linkedin first we need to login to linkedin. There are two ways of login:-
* Credentials
* Cookies
...
To fetch linkedin feed we use ``get_feed`` method.
It does not require any input parameter. 
It expects feed to be opened in automated browser
